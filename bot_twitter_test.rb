...
def updateClient
	...
	client.update("Bonjour monde @tony_thp! avec Argv")
end
...
def directMessage
	...
	target = "@BlierBea"
	client.create_direct_message(target, "Coucou, ca va toi?")
end
...
def target
 	...
 	friends = ["@BlierBea", "@Shownerot", "@tony_thp"]
 	friends.each do |x|
 		client.update("Bonjour #{x}, je suis élève à The Hacking Projet une formation gratuite au code et je viens de faire un bot Twitter trop cool !!")
 	end	
end

def all()
    puts "Que faire?  update stream"
    what_to_do = STDIN.gets.chomp.to_s
    case what_to_do
    when "update" then updateClient()
    when "stream" then stream()
    when "direct" then directMessage()
